...
public class BlockUtil1_17_R1 extends IBlockUtil1_13<BlockState> {
    @Override
    ...
    public void setNMS(@NotNull Block b, @NotNull BlockState ibd) {
        ...
        BlockState old = chunk.getTypeIfLoaded(bp); // should be fine (and faster) to use this method
        try {
            // doPlace = false -> don't simulate player block place (should be faster)
            chunk.setType(bp, ibd, false, false);
        } catch (Throwable t) {
            t.printStackTrace();
        }
        w.sendBlockUpdated(bp, old, ibd, 3);
    }
}
