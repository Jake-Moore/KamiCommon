...
public class BlockUtil1_9_R2 extends AbstractBlockUtil {
    @Override
    public void setBlock(@NotNull Block b, @NotNull XBlockData blockData, @NotNull PlaceType placeType) {
        ...
        if (placeType == PlaceType.BUKKIT) {
            ...
            IBlockData old = net.minecraft.server.v1_9_R2.Block.getByCombinedId(legacyGetCombined(b.getType().getId(), b.getData()));
            IBlockData ibd = net.minecraft.server.v1_9_R2.Block.getByCombinedId(legacyGetCombined(material, data));
            try {
                chunk.a(bp, ibd);
            } catch (Throwable t) {
                t.printStackTrace();
