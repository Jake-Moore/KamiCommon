...
public class Chunk_1_19_R3 implements NMSChunkDef {
    private final @NotNull ChunkProvider_1_19_R3 provider;
    private final @NotNull Chunk bukkitChunk;
    private final @NotNull ChunkAccess chunk;
    public Chunk_1_19_R3(@NotNull ChunkProvider_1_19_R3 provider, @NotNull Chunk bukkitChunk) {
    ...
    public @NotNull NMSChunkSection getSection(int y) {
        return new ChunkSection_1_19_R3(this, this.chunk, y);
    }
    ...
    public void sendUpdatePacket(@NotNull Player player) {
        if (this.chunk instanceof LevelChunk levelChunk) {
            Packet<?> packet = new ClientboundLevelChunkWithLightPacket(levelChunk, levelChunk.getLevel().getLightEngine(), null, null, true, true);
