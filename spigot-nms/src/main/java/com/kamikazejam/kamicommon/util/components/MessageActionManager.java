...
import com.kamikazejam.kamicommon.util.components.actions.Click;
import com.kamikazejam.kamicommon.util.components.actions.Hover;
    ...
    private static List<BaseComponent> getComponentsWithButton(Action action, BaseComponent component) {
        ...
        for (BaseComponent clickComponent : legacyTexts) {
            if (action instanceof Click) {
                ((Click) action).addClickEvent(clickComponent);
            }
            if (action instanceof Hover) {
                ((Hover) action).addHoverEvent(clickComponent);
            }
        }
