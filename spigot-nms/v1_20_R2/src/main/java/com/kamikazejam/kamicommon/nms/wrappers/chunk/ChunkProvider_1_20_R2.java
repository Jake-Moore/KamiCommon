...
public class ChunkProvider_1_20_R2 implements NMSChunkProvider {
    ...
    public ChunkProvider_1_20_R2(@NotNull ServerChunkCache server) {
        this.handle = server;
    }
    ...
    public @NotNull NMSChunk getOrCreateChunk(int x, int z) {
        return new Chunk_1_20_R2(Objects.requireNonNull(this.handle.getChunkAtMainThread(x, z)));
    }
    ...
    public @NotNull NMSChunk wrap(org.bukkit.@NotNull Chunk chunk) {
        return new Chunk_1_20_R2(((CraftChunk) chunk).getHandle(ChunkStatus.FULL));
    }
}
