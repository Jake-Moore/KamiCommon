...
public class BlockUtil1_12_R1 extends AbstractBlockUtil {
    ...
    public void setBlock(@NotNull Block b, @NotNull XBlockData blockData, @NotNull PlaceType placeType) {
        ...
        if (placeType == PlaceType.BUKKIT) {
            ...
            IBlockData old = net.minecraft.server.v1_12_R1.Block.getByCombinedId(legacyGetCombined(b.getType().getId(), b.getData()));
            IBlockData ibd = net.minecraft.server.v1_12_R1.Block.getByCombinedId(legacyGetCombined(material, data));
            try {
                chunk.a(bp, ibd);
            } catch (Throwable t) {
                t.printStackTrace();
