...
public class KamiCommand implements Active, PluginIdentifiableCommand {
		...
		try {
			...
			if (this.isParent() && !args.isEmpty()) {
				...
				// Crap!
					...
					base = base.replace(Lang.COMMAND_REPLACEMENT, ChatColor.AQUA + token);
					...
					KMessage help = new KMessageSingle(Lang.COMMAND_CHILD_HELP).addClickSuggestCommand(
									Lang.COMMAND_REPLACEMENT,
	...
	public boolean isArgsValid(@NotNull List<String> args, CommandSender sender) {
		if (args.size() < this.getParameterCountRequired(sender)) {
			if (sender != null) {
				sender.sendMessage(StringUtil.t(Lang.COMMAND_TOO_FEW_ARGUMENTS));
		...
		if (args.size() > this.getParameterCount(sender) && this.isOverflowSensitive()) {
			if (sender != null) {
				...
				sender.sendMessage(StringUtil.t(Lang.COMMAND_TOO_MANY_ARGUMENTS2));
				sender.sendMessage(StringUtil.t(this.getTemplate(false, false, false, sender)));
			}
			return false;
		}
	...
	public static final String TEMPLATE_CORE = ChatColor.AQUA + "/";
	...
	public String getTemplate(boolean addDesc, boolean onlyFirstAlias, boolean onlyOneSubAlias, @Nullable CommandSender sender) {
		...
		for (String parameter : this.getTemplateParameters(sender)) {
			ret.append(" ").append(ChatColor.DARK_AQUA).append(parameter);
		}

		// Add desc
		if (addDesc) {
			ret.append(" ").append(ChatColor.YELLOW).append(this.getDesc());
		}
	...
	@NotNull
	public String getTemplateWithArgs(CommandSender sender, String... args) {
		return this.getTemplateWithArgs(sender, KUtil.list(args));
	}

	@NotNull
	public String getTemplateWithArgs(CommandSender sender, @NotNull List<String> args) {
		StringBuilder ret = new StringBuilder(this.getTemplateChain(true, sender));

		for (String arg : args) {
			ret.append(" ");
			ret.append(ChatColor.DARK_AQUA).append(arg);
		}
	...
	@NotNull
	public String getTemplateChain(boolean onlyFirstAlias, CommandSender sender) {
	...
	public String getTemplateChain(boolean onlyFirstAlias, boolean onlyOneSubAlias, @Nullable CommandSender sender) {
		StringBuilder ret = new StringBuilder(TEMPLATE_CORE);
		...
		for (KamiCommand command : commands) {
			...
			if (sender != null && !command.isRequirementsMet(sender, false)) {
				base = ChatColor.RED + base;
			} else {
				base = ChatColor.AQUA + base;
			}
	...
	public void setHelp(@NotNull List<KMessageSingle> help) {
		this.help = help;
	}
	public void setHelp(@NotNull KMessageBlock block) {
		this.help.clear();
		for (String line : block.getLines()) {
			this.help.add(new KMessageSingle(line));
		}
	}
}
