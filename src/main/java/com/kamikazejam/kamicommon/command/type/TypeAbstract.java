...
import com.kamikazejam.kamicommon.command.editor.*;
...
import com.kamikazejam.kamicommon.util.interfaces.Colorized;
...
public abstract class TypeAbstract<T> implements Type<T> {
	...
	protected List<Type<?>> innerTypes = new KamiList<>();
	...
	public <I extends Type<?>> I getInnerType(int index) {
		return (I) this.getInnerTypes().get(index);
	}
	...
	public void setInnerTypes(Collection<Type<?>> innerTypes) {
		this.innerTypes = new KamiList<>(innerTypes);
	}

	@Contract(mutates = "this")
	@Override
	public void setInnerTypes(Type<?> @NotNull ... innerTypes) {
		this.setInnerTypes(Arrays.asList(innerTypes));
	}

	@Contract(mutates = "this")
	@Override
	public void setInnerType(Type<?> innerType) {
		this.setInnerTypes(innerType);
	}
	...
	@Contract(mutates = "this")
	@Override
	public void setUserOrder(Integer @NotNull ... userOrder) {
		this.setUserOrder(Arrays.asList(userOrder));
	}

	@Override
	public List<Integer> getUserOrder() {
		if (this.userOrder == null) {
			this.userOrder = KUtil.range(0, this.getInnerTypes().size());
		}
	...
	protected List<Property<T, ?>> innerProperties = new KamiList<>();

	public boolean hasInnerProperties() {
		return !this.getInnerProperties().isEmpty();
	}

	@SuppressWarnings("unchecked")
	public <I extends Property<T, ?>> List<I> getInnerProperties() {
		return (List<I>) this.innerProperties;
	}

	@SuppressWarnings("unchecked")
	public <I extends Property<T, ?>> I getInnerProperty(int index) {
		return (I) this.getInnerProperties().get(index);
	}

	@Contract(mutates = "this")
	public <I extends Property<T, ?>> void setInnerProperties(Collection<I> innerProperties) { this.innerProperties = new KamiList<>(innerProperties); }
	@SafeVarargs
	@Contract(mutates = "this")
	...
	@Contract(mutates = "this")
	public void setVisualMsonOverridden(boolean visualMsonOverridden) {
	...
	@Override
	public T read(CommandSender sender) throws KamiCommonException {
		return this.read(null, sender);
	}

	@Override
	public T read(String arg) throws KamiCommonException {
		return this.read(arg, null);
	}

	@Override
	public T read() throws KamiCommonException {
	...
	public void setContainerComparator(Comparator<?> comparator) {
		this.elementComparator = (Comparator<Object>) comparator;
	}
	...
	public <E> List<E> getContainerElementsOrdered(Iterable<E> elements) {
		...
		if (elements instanceof Collection<?>) {
			ret = new KamiList<>((Collection<E>) elements);
		} else {
			ret = new KamiList<>();
